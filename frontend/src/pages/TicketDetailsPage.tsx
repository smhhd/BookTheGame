...
export function TicketDetailsPage() {
  ...
  return <div className="page details-page">
    ...
    <section className="details-grid">
      <article className="panel"><h2>اطلاعات مسابقه</h2><dl className="details-list"><div><dt>تاریخ و ساعت</dt><dd>{formatDate(ticket.match_datetime)}</dd></div><div><dt>محل برگزاری</dt><dd>{ticket.venue_name}</dd></div><div><dt>شهر</dt><dd>{ticket.city_name}</dd></div><div><dt>رده بلیت</dt><dd>{ticket.category_name}</dd></div><div><dt>جایگاه</dt><dd>{ticket.section_name || "—"}</dd></div><div><dt>ردیف / صندلی</dt><dd>{ticket.row_number || "—"} / {ticket.seat_number || "—"}</dd></div></dl></article>
